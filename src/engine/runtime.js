...
const defaultBlockPackages = {
    ...
    scratch3_wikipedia: require('../blocks/scratch3_wikipedia')
};
...
class Runtime extends EventEmitter {
    ...
    _removeExtensionPrimitives(extensionId) {
        ...
        if (categoryInfo.blocks) {
            for (const block of categoryInfo.blocks) {
                if (block && block.info && block.info.opcode) {
                    const opcode = `${categoryInfo.id}_${block.info.opcode}`;
        ...
        const opcodePrefix = `${categoryInfo.id}_`;
        this._monitorState.filter(monitor => {
            const opcode = monitor.opcode;
            return !(typeof opcode === 'string' && opcode.startsWith(opcodePrefix));
        });

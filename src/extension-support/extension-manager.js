...
const defaultBuiltinExtensions = {
    ...
    imagem: () => require('../extensions/scratch3_imagem'),
    capib: () => require('../extensions/scratch3_capivarablock'),
    // tw: core extension
...
class ExtensionManager {
    ...
    isRemovableExtension(extensionId) {
        if (extensionId === 'tw') return false;
        return this.isExtensionLoaded(extensionId);
    }

    removeExtension(extensionId) {
        if (!this.isRemovableExtension(extensionId)) {
            log.warn(`Cannot remove extension ${extensionId}: not loaded or not removable.`);
            return false;
        }
        const serviceName = this._loadedExtensions.get(extensionId);
        dispatch.call('runtime', '_removeExtensionPrimitives', extensionId).catch(e => {
            log.error(`Failed to remove primitives for extension ${extensionId}:`, e);
        });
        this._loadedExtensions.delete(extensionId);
        if (serviceName) dispatch.removeServiceSync(serviceName);
        return true;
    }
